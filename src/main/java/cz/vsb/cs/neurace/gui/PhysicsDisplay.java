...
/**
 * 3D zobrazení fyzikálních těles. Otevírá se ve vlastním okně.
 */
public class PhysicsDisplay implements Runnable{
	...
	protected DynamicsWorld dynamicsWorld = null;
        protected ArrayList<PhysicalCar>  cars;
	...
	protected float ShootBoxInitialSpeed = 30f;
	...
	public PhysicsDisplay(DynamicsWorld dynamicsWorld, ArrayList<PhysicalCar>  cars, int fps) {
            this.gl = new LwjglGL();
            ...
            limitFps = true;
            cameraHeight = 10f;
            minCameraDistance = 15f;
            maxCameraDistance = 20f;
        ...
        @Override
        public void run() {
            String title =  Text.getString("3D_title");
            ...
            try {
                ...
                Logger.getLogger(RacePhysics.class.getName()).log(Level.SEVERE, null, e);
            ...
            while (!Display.isCloseRequested() && !quit) {
                ...
                while (Keyboard.next()) {
                        if (Keyboard.getEventCharacter() != '\0') {
                                this.keyboardCallback(Keyboard.getEventCharacter(), Mouse.getX(), Mouse.getY(), modifiers);
                        }

                        if (Keyboard.getEventKeyState()) {
                                this.specialKeyboard(Keyboard.getEventKey(), Mouse.getX(), Mouse.getY(), modifiers);
                        }
                        else {
                                this.specialKeyboardUp(Keyboard.getEventKey(), Mouse.getX(), Mouse.getY(), modifiers);
                        }
                ...
                while (Mouse.next()) {
                        if (Mouse.getEventButton() != -1) {
                                ...
                                this.mouseFunc(btn, Mouse.getEventButtonState()? 0 : 1, Mouse.getEventX(), Display.getDisplayMode().getHeight()-1 - Mouse.getEventY());
                        }
                        this.mouseMotionFunc(Mouse.getEventX(), Display.getDisplayMode().getHeight()-1 - Mouse.getEventY());
                }
	...
	public void myinit() {
		...
		//gl.glClearColor(0.7f, 0.7f, 0.7f, 0f);
                gl.glClearColor(0.7f, 0.8f, 1.0f, 0f);
	...
	private Vector3f camToObject = new Vector3f();
	private Vector3f tmp = new Vector3f();
//	private Vector3f eyePos = new Vector3f();
	private Vector3f forward = new Vector3f();
	private Vector3f right = new Vector3f();
	public void updateCamera() {
            if(follow && !cars.isEmpty()) {
		...
		// look at the vehicle
                if(carCameraNum >= cars.size()) {
                   carCameraNum = cars.size()-1;
                }
		cars.get(carCameraNum).getRaycastVehicle().getRigidBody().getMotionState().getWorldTransform(chassisWorldTrans);
		...
		cameraDistance = camToObject.length();
	...
	public void shootBox(Vector3f destination) {
		if (dynamicsWorld != null) {
			...
			RigidBody body = RacePhysics.createRigidBody(mass, startTransform, shootBoxShape, dynamicsWorld);
	...
	private Vector3f rayFrom = new Vector3f(getCameraPosition());
	private Vector3f rayForward = new Vector3f();
	private Vector3f rightOffset = new Vector3f();
	private Vector3f vertical = new Vector3f();
	private Vector3f hor = new Vector3f();
	private Vector3f rayToCenter = new Vector3f();
	private Vector3f dHor = new Vector3f();
	private Vector3f dVert = new Vector3f();
					...
					if (dynamicsWorld != null) {
						...
						if (rayCallback.hasHit()) {
							...
							if (body != null) {
								...
								relPos.sub(rayCallback.hitPointWorld, body.getCenterOfMassPosition(tmp1));
	...
	private Vector3f newRayTo = new Vector3f();
	private Vector3f eyePos = new Vector3f();
	public void mouseMotionFunc(int x, int y) {
		if (pickConstraint != null) {
			...
			if (p2p != null) {
				...
				newRayTo.set(getRayTo(x, y));
				eyePos.set(cameraPosition);
	...
	public void renderme() {
		if (dynamicsWorld != null) {
                    ...
                    //if(!cars.isEmpty()) {
                        RaycastVehicle vehicle = car.getRaycastVehicle();

                        float wheelRadius = car.getWheelRadius(); //0.5f
                        float wheelWidth = car.getWheelWidth(); //0.4f
                        ...
                        for (int i = 0; i < vehicle.getNumWheels(); i++) {
                                ...
                                MyShapeDrawer.drawOpenGL(gl, trans, wheelShape, wheelColor, getDebugMode());
                        }
                     //}
                    }
			...
			for (int i = 0; i < numObjects; i++) {
				...
				if(colObj != null) {
                                    ...
                                    if(i == 0) {
                                        wireColor.set(0.2f, 0.8f, 0.2f);
                                    }

                                    MyShapeDrawer.drawOpenGL(gl, m, colObj.getCollisionShape(), wireColor, getDebugMode());
        ...
        public void jump(float force) {
            this.cars.get(0).getRaycastVehicle().getRigidBody().applyImpulse(
                    new Vector3f(0, force, 0), new Vector3f(0,0,0));
        }
